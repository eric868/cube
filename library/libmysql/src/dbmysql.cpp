...
namespace database
{
    ...
    int db_mysql::open(const unidb_param& params)
    {
        ...
        if (mysql_init(&mysql_) == NULL) {
            STLOG_ERROR << "database" << NAUT_DATABASE_E_INIT_MYSQL_FAILED <<
                " initialize mysql failed, error:" << mysql_error(&mysql_);
        ...
        if (mysql_real_connect(&mysql_, params_.host.c_str(),
            ...
            STLOG_ERROR << "database" << NAUT_DATABASE_E_CONNECT_TO_MYSQL_FAILED <<
                " connect to mysql failed, error:" << mysql_error(&mysql_);
    ...
    {
        if (conn_count_ > 0) {
            STLOG_ERROR << "database" << NAUT_DATABASE_E_ALL_CONN_SHOULD_RELEASE <<
    ...
    bool db_mysql::connected()
    {
        ...
        if (ret != 0) {
            STLOG_ERROR << "database" << NAUT_DATABASE_E_PING_MYSQL_FAILED <<
                " ping mysql failed, error:" << ret << " msg:" << mysql_error(&mysql_);
            return false;
        }
        return true;
    }
    ...
    {
        if (!is_open_) {
            STLOG_WARN << "database" << "the database has not been initialized";
            return "";
        }
    ...
    int db_mysql::create_database_if_not_exists(const char* database_name)
    {
        ...
        if (mysql_init(&mysql) == NULL) {
            STLOG_ERROR << "database" << NAUT_DATABASE_E_INIT_MYSQL_FAILED <<
                "initialize mysql failed, error:" << mysql_error(&mysql);
            ASSIGN_AND_CHECK_LABEL(last_error_, NAUT_DATABASE_E_INIT_MYSQL_FAILED, end);
        }
        ...
        if (mysql_real_connect(&mysql, params_.host.c_str(),
            ...
            STLOG_ERROR << "database" << NAUT_DATABASE_E_CONNECT_TO_MYSQL_FAILED <<
                "connect to mysql failed, error:" << mysql_error(&mysql);
            ASSIGN_AND_CHECK_LABEL(last_error_, NAUT_DATABASE_E_CONNECT_TO_MYSQL_FAILED, end);
        }
        ...
        if (ret != 0) {
            STLOG_ERROR << "database" << NAUT_DATABASE_E_MYSQL_EXECUTE_SQL_FAILED <<
                "execute sql:" << buffer <<" failed, error:" << ret << " ,"
                << mysql_error(&mysql);
    ...
    {
        if (!initialized_) {
            STLOG_WARN << "database mysql connection is not initialized or initialize failed";
            return false;
        }
        ...
        if (ret) {
            STLOG_ERROR << "database" << NAUT_DATABASE_E_PING_MYSQL_FAILED <<
                "ping mysql failed, error:" << ret << " ,"<< mysql_error(&mysql_);
    ...
    int db_mysql_conn::real_connect()
    {
        ...
        if (ret != 0) {
            STLOG_ERROR << "database" << NAUT_DATABASE_E_MYSQL_SET_CHARSET_FAILED <<
                "set mysql charset failed:" << ret << " ," << mysql_error(&mysql_);
            ASSIGN_AND_CHECK_LABEL(ret, NAUT_DATABASE_E_MYSQL_SET_CHARSET_FAILED, end);
        }

        ret = mysql_autocommit(&mysql_, true);
        if (ret != 0) {
            STLOG_ERROR << "database" << NAUT_DATABASE_E_MYSQL_SET_AUTOCOMMIT_FAILED <<
                "mysql set auto commit failed, error:" << ret << " ," << mysql_error(&mysql_);
    ...
    bool db_mysql_conn::begin_transaction()
    {
        ...
        if (ret != 0) {
            STLOG_ERROR << "database" << NAUT_DATABASE_E_MYSQL_BEGIN_TRANSACTION_FAILED <<
                "begin transaction failed, error:" << ret << " ," << last_error_inner_
                << " ," << slast_error_inner_.c_str();
    ...
    bool db_mysql_conn::commit()
    {
        ...
        if (ret != 0) {
            STLOG_ERROR << "database" << NAUT_DATABASE_E_MYSQL_COMMIT_FAILED <<
                "mysql commit transaction failed, error:" << ret << " ," << mysql_error(&mysql_);
    ...
    bool db_mysql_conn::rollback()
    {
        ...
        if (ret != 0) {
            STLOG_ERROR << "database" << NAUT_DATABASE_E_MYSQL_COMMIT_FAILED <<
                "mysql rollback transaction failed, error:" << ret << "," << mysql_error(&mysql_);
    ...
    bool db_mysql_conn::execute(const char* sql)
    {
        ...
        if (ret != 0) {
            STLOG_ERROR << "database" << NAUT_DATABASE_E_MYSQL_EXECUTE_SQL_FAILED <<
                "execute sql:" << sql << "failed, error:" << ret << ","
                << last_error_inner_ << "," << slast_error_inner_.c_str();
    ...
    std::vector<std::string> db_mysql_conn::get_fields()
    {
        ...
        std::unordered_map<const char*, int, string_hash, string_compare>::iterator it = map_fields_.begin();
    ...
    int64_t db_mysql_conn::get_int64(int id)
    {
        assert(id >= 0 && id < num_cols_);
    ...
    int64_t db_mysql_conn::get_int64(const std::string& field_name)
    {
        return get_int64(field_name.c_str());
    }

    int64_t db_mysql_conn::get_int64(const char* field_name)
    {
        int index = map_fields_[field_name] - 1;
        if (index >= 0) {
